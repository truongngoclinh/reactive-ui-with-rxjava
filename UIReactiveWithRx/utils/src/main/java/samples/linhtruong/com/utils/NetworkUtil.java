...
public class NetworkUtil {
    ...
    public static Result check() {
        return new Result(getNetworkInfo(sCtx), isDozing(sCtx));
    }

    private static NetworkInfo getNetworkInfo(Context context) {
        ConnectivityManager cm = ((ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE));
        if (cm == null) {
            LogUtils.e("checkNetwork: ConnectivityManager is null !!!");
            return null;
        }
        NetworkInfo ni = cm.getActiveNetworkInfo();
        if (ni == null) {
            LogUtils.w("checkNetwork: networkInfo is null");
            return null;
        }
        LogUtils.d("checkNetwork: state=%s type=%s connected=%s dozing=%s", ni.getState(), ni.getTypeName(),
                ni.isConnected(), isDozing(context));
        return ni;
    }
    ...
    public final static class Result {
        ...
        private final boolean isDozing;

        public Result(NetworkInfo info, boolean isDozing) {
            this.info = info;
